...
echo "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaafaffff";
	if ($result_of_check->num_rows != 0) {
		...
		<div class="panel panel-danger">
			<div class="panel-heading">日期为<strong>'.$date.'</strong> ，楼层为<strong>'.$region.$build_num.'#'.$part.'区'.$floor.'层</strong>的记录已存在！ 如下表所示：</div>
		...
		if($part==0){
			...
			<div class="panel panel-info">
				<div class="panel-heading">
					...
					楼层为：
					<strong class="text-danger">'.$region.'</strong>苑
					<strong class="text-danger">'.$build_num.'</strong>号楼

					<strong class="text-danger">'.$floor.'</strong>层
				</div>
				<div class="panel-body">
					';
			...
			<div class="panel panel-info">
				<div class="panel-heading">
					...
					<strong class="text-danger">'.$region.'</strong>苑
					<strong class="text-danger">'.$build_num.'</strong>号楼
					<strong class="text-danger">'.$part.'</strong>区
					<strong class="text-danger">'.$floor.'</strong>层
				</div>
				<div class="panel-body">
					';
			...
			//per row define
							echo "<tr>";
					//dorm_num
							echo "<td><input name='dorm_num$i' class='form-control' type='text' value='" . $row['dorm_num'] . "' readonly /></td>";
					//wc_balcony
							echo "<td>";
...
elseif(isset($_POST["add_step2"])){
	...
	$sql_insert="INSERT INTO routine_add(date,add_floor,add_time,user_name) VALUES ('$date','$add_floor','$add_time','$user_name')";
	$db->query($sql_insert) or die($db->error);
	//log
	$action="增加记录, 日期：".$date." , 楼层：".$add_floor;
	add_log($action,$db);
