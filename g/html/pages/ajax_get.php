        ...
        echo '<table class="table table-bordered _responsive-utilities build_model" >
            ...
            while($row_floor = $result_floor->fetch_array(MYSQLI_ASSOC)){
                ...
                if ($result_of_check->num_rows != 0){
					...
					echo "<a href='#sec2'><button name = 'add_step1' class='btn btn-success' value = '$str' style='width: 100%;' onclick='view_display(this.value)'>".$floor."层</button></a>";
					...
					//echo "<p>$this_floor</p>";
					//$str="bbbbbbbbbbbb";
					echo "<a href='#sec2'><button name = 'add_step1' class='btn btn-default' value = '$str' style='width: 100%;' onclick='view_display(this.value)'>".$floor."层</button></a>";
        ...
        echo '<table class="table table-bordered _responsive-utilities build_model" >
            ...
            while($row_floor = $result_floor->fetch_array(MYSQLI_ASSOC)){
                ...
                if ($result_of_check->num_rows != 0){
                    echo '<tr><td class="is_green">';
                    $str="date=$date&region=$region&build_num=$build_num&part=$part&floor=$floor";
                    echo "<a href='#sec2'><button name = 'add_step1' class='btn btn-success' value = '$str' style='width: 100%;' onclick='view_get_dorm(this.value)'>".$floor."层</button></a>";
                    ...
                    //echo "<p>$this_floor</p>";
                    //$str="bbbbbbbbbbbb";
                    echo "<a href='#sec2'><button name = 'add_step1' class='btn btn-default' value = '$str' style='width: 100%;' onclick='view_get_dorm(this.value)'>".$floor."层</button></a>";
...
<strong class="bg-success text-success">绿色</strong>代表有记录  <strong>0区</strong>代表此楼不分AB区)</p> ';
	...
	while($row = $result->fetch_array(MYSQLI_ASSOC)){
		...
		if ($result_of_check->num_rows != 0){
			...
			echo "<a href='#sec3'><button class='btn btn-success' value='".$dorm_num_replace."' onclick='view_dorm_routine(this.value)'>$dorm_num</button></a>";
			$i=$i+1;
		}else{
			echo "<a href='#sec3'><button class='btn btn-default' value='".$dorm_num_replace."' onclick='view_dorm_routine(this.value)'>$dorm_num</button></a>";
...
elseif ($_GET["view_display_stu"]=="1"){
